...
class DataConfig:
    ...
    chunk_size: int | None = None  # If set, chunk long sequences
    overlap_size: int = 128  # Overlap between chunks
    num_workers: int = 4
...
class TokenDataset(Dataset):
    ...
    def __init__(
        ...
        # Load or create file cache
        self.files = get_all_xml_paths()

        # If chunking is enabled, pre-compute chunks
        self.chunks: Optional[List[Tuple[int, int, int]]] = None
        if config.chunk_size is not None:
            self._prepare_chunks()

    def _prepare_chunks(self) -> None:
        """Pre-compute chunk indices for all files"""
        assert self.config.chunk_size is not None, "Chunk size must be set to prepare chunks"
        self.chunks = []
        ...
        for file_idx, file in enumerate(tqdm(self.files)):
            ...
            for start_idx in range(0, seq_len, stride):
                end_idx = min(start_idx + self.config.chunk_size, seq_len)
                self.chunks.append((file_idx, start_idx, end_idx))
    ...
    def __getitem__(self, idx: int) -> Dict[str, Union[torch.Tensor, int]]:
        if self.chunks is not None:
            # Load a chunk
            file_idx, start_idx, end_idx = self.chunks[idx]
            ...
            if data.shape[0] > self.config.max_seq_length:
                if self.is_training:
                    # Random crop for training
                    start = np.random.randint(0, data.shape[0] - self.config.max_seq_length + 1)
                    data = data[start:start + self.config.max_seq_length]
                else:
                    # Take first max_seq_length tokens for validation
                    data = data[:self.config.max_seq_length]
...
class TokenDataModule:
    ...
    def __init__(
        ...
        full_dataset = TokenDataset(config, load_fn, is_training=True)
        ...
        # Create datasets with split files
        self.train_dataset = self._create_subset_dataset(
            full_dataset.files, train_indices, is_training=True
        )
        self.val_dataset = self._create_subset_dataset(
            full_dataset.files, val_indices, is_training=False
        )
        self.test_dataset = self._create_subset_dataset(
            full_dataset.files, test_indices, is_training=False
        )
    ...
    def _create_subset_dataset(
        ...
        """Create a dataset from a subset of files"""
        subset_files = [all_files[i] for i in indices]

        # Create temporary dataset with subset
        dataset = TokenDataset(
            self.config, self.load_fn, is_training
        )
        dataset.files = subset_files

        # Re-prepare chunks if necessary
        if self.config.chunk_size is not None:
            dataset._prepare_chunks()
